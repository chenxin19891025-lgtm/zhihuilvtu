/**
 * Cloudflare Worker: 智绘旅途
 * 处理 /api/generate API 请求，其余路由服务静态文件
 */
...
async function handleGenerate(request, env) {
  ...
  if (!apiKey) {
    return new Response(
      JSON.stringify({ error: '未配置 DEEPSEEK_API_KEY，请在 Cloudflare Worker 设置中添加环境变量' }),
...
返回JSON格式如下：
{
  ...
  try {
    ...
    const msg = error.name === 'AbortError' ? 'AI响应超时，请稍后重试' : (error.message || '生成失败，请稍后重试');
    return new Response(
